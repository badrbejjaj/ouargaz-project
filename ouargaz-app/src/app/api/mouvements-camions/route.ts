...
async function notify(role: string, title: string, message: string, link = '/mouvements-camions') {
  const notif = await prisma.notification.create({ data: { role, title, message, link } })
  await triggerBroadcast('NOTIFICATION_UPDATE', notif, role)
}
async function notifyBackoffice(title: string, message: string, link = '/mouvements-camions') {
  const roles = ['CHEF_CENTRE', 'ADJOINT_CHEF_CENTRE', 'ADMINISTRATIF', 'AGENT_SAISIE', 'CONSULTATION']
  await Promise.all(roles.map(role => notify(role, title, message, link)))
}
async function notifyAll(title: string, message: string, link = '/mouvements-camions') {
  await Promise.all([
    notifyBackoffice(title, message, link),
...
export async function PUT(req: NextRequest) {
  ...
  if (action === 'sortir') {
    ...
    const tr12 = camion.def_traitees_12kg || camion.def_rendues_12kg, tr6 = camion.def_traitees_6kg || camion.def_rendues_6kg, tr3 = camion.def_traitees_3kg || camion.def_rendues_3kg
    const data: any = {
      statut: STATUTS.SORTI, sortieAt: new Date(), exitedBy: session.username, numero_bl_sortie,
